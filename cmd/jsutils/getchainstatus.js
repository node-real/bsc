...
async function getPerformanceData()  {
    ...
    for (let i = program.startNum; i < program.endNum; i++) {
        ...
        let timestamp = eval(eval(header.timestampInMilliseconds).toString(10))
        if (parliaEnabled) {
    ...
    let startTime = eval(eval(startHeader.timestampInMilliseconds).toString(10))
    let endHeader = await provider.send("eth_getHeaderByNumber", [
        ethers.toQuantity(program.endNum)]);
    let endTime = eval(eval(endHeader.timestampInMilliseconds).toString(10))
