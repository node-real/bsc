...
func NewInspector(trieDB *Database, tr *Trie, blocknum uint64, jobnum uint64) (*Inspector, error) {
	if tr == nil {
...
func (inspect *Inspector) Run() {
	...
	go func() {
		...
		for {
			select {
			case <-ticker.C:
				inspect.trieDB.Cap(DEFAULT_TRIEDBCACHE_SIZE)
			}
		}
	}()
...
func (inspect *Inspector) ConcurrentTraversal(theTrie *Trie, theTrieTreeStat *TrieTreeStat, theNode node, height uint32, path []byte) {
	...
	case hashNode:
		n, err := theTrie.resolveHash(current, path)
		if err != nil {
	...
	case valueNode:
		...
		root, _ := theTrie.root.cache()
		contractTrie, err := New(StorageTrieID(common.BytesToHash(root), common.BytesToHash(hexToKeybytes(path)), account.Root), inspect.trieDB)
		if err != nil {
